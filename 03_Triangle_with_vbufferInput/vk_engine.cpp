...
uint32_t VulkanEngine::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
	...
	vkGetPhysicalDeviceMemoryProperties(m_chosenGPUPhysicalDevice, &memProperties);
	...
	{
		if ((typeFilter & (1 << i)) //checking if corresponding bit is set to 1
...
void VulkanEngine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
	...
	vkCmdDraw(m_commandBuffers[currentFrame], static_cast<uint32_t>(m_vertices.size()), 1, 0, 0);
...
void VulkanEngine::createGraphicsPipeline()
{
	...
	*/
	...
	//if we are giving vertices in vertex shader
	...
	//if we are giving vertices from cpu side
	...
	void* pUserData)
{
	...
	if (firstTime)
	{
		...
		fprintf(gpFile1, "validation layer: Started Printing....\n");
		fclose(gpFile1);
		firstTime = false;
	}
